...
package com.oltpbenchmark.benchmarks.lsd.tpcc;
...
public class TPCCUtil {
    ...
    public static FutureCustomer newFutureCustomerFromResults(FutureResultSet rs)
            throws SQLException {
        FutureCustomer c = new FutureCustomer();
        // TODO: Use column indices: probably faster?
        c.c_first = rs.getFutureString("c_first");
        c.c_middle = rs.getFutureString("c_middle");
        c.c_street_1 = rs.getFutureString("c_street_1");
        c.c_street_2 = rs.getFutureString("c_street_2");
        c.c_city = rs.getFutureString("c_city");
        c.c_state = rs.getFutureString("c_state");
        c.c_zip = rs.getFutureString("c_zip");
        c.c_phone = rs.getFutureString("c_phone");
        c.c_credit = rs.getFutureString("c_credit");
        c.c_credit_lim = rs.getFutureFloat("c_credit_lim");
        c.c_discount = rs.getFutureFloat("c_discount");
        c.c_balance = rs.getFutureFloat("c_balance");
        c.c_ytd_payment = rs.getFutureFloat("c_ytd_payment");
        c.c_payment_cnt = rs.getFutureInt("c_payment_cnt");
        c.c_since = rs.getFutureTimestamp("c_since");
        return c;
    }
    ...
    public static <T> Future<T> newFuture(Action<T> action) {
        return new Future<T>() {
            @Override
            public T resolve() {
